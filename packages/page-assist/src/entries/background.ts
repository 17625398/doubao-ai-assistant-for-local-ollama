...
import { initOpencliBridge, execLocalOpencliCommand } from "@/libs/opencli/bridge"
...
export default defineBackground({
  main() {
    ...
    const initialize = async () => {
      try {
        storage.watch({
          ...
          opencliBridgeEnabled: async (value) => {
            const enabled = value?.newValue || false
            if (enabled) {
              await initOpencliBridge()
            }
          }
        })
        ...
        try {
          const opencliEnabled = await storage.get("opencliBridgeEnabled")
          if (opencliEnabled) {
            await initOpencliBridge()
          }
        } catch {}
    ...
    browser.runtime.onMessage.addListener(async (message, sender) => {
      if (message?.type === "opencli_bridge_exec") {
        try {
          const res = await execLocalOpencliCommand(message.command)
