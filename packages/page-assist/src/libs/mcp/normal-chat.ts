...
export const runMcpNormalChatMode = async (
  ...
  try {
    ...
    try {
      const { isOpencliBridgeEnabled } = await import("./tools/opencli-tools")
      if (await isOpencliBridgeEnabled()) {
        const { createOpencliTools } = await import("./tools/opencli-tools")
        const localOpencliTools = createOpencliTools()
        for (const t of localOpencliTools) tools.push(t)
      }
    } catch {}
