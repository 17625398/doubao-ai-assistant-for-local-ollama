...
const ChatAreaComponent: React.FC<ChatAreaProps> = ({ chatArea }) => {
  ...
  const { chatInputHeight, chatInputContainerRef, isImagenModel, handleQuote, handleInsert } =
    useChatArea({
      currentChatSettings: session.currentChatSettings,
    })

  const newChatShortcut = useMemo(
    () => getShortcutDisplay('general.newChat', appSettings),
    [appSettings]
  )
  const pipShortcut = useMemo(
    () => getShortcutDisplay('general.togglePip', appSettings),
    [appSettings]
  )
  ...
  return (
    <div
      className="flex flex-col flex-grow h-full overflow-hidden relative chat-bg-enhancement max-w-full"
    ...
    >
      ...
      <div className="flex flex-col flex-grow h-full overflow-hidden mx-auto w-full max-w-[900px] lg:max-w-[1000px]">
        <Header
          onNewChat={header.onNewChat}
        ...
        <ChatAreaProvider value={providerValue}>
          ...
          >
            <div className="pointer-events-auto mx-auto max-w-[900px] lg:max-w-[1000px] px-4 sm:px-6 lg:px-8">
              <ChatInput />
            </div>
          </div>
        </ChatAreaProvider>
