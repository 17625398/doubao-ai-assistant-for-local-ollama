...
export const OllamaSettingsDialog: React.FC<OllamaSettingsDialogProps> = ({
  open,
  settings,
  draft,
  onDraftChange,
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          <div>
            ...
            <p className="mt-1.5 text-[11px] text-[var(--text-tertiary)]">
              默认走 /api/ollama 代理；填写本地或局域网地址时也会由服务端代理转发。
            </p>
          </div>
          ...
          <div>
            ...
            {models.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-[13px] font-medium truncate ${isSelected ? 'text-[var(--brand-orange)]' : 'text-[var(--text-primary)]'}`}
                          >
                            {model.name}
                          </span>
              ...
              <div className="rounded-2xl border border-dashed border-[var(--border-light)] bg-[var(--bg-secondary)] px-3 py-6 text-center text-[12px] text-[var(--text-tertiary)]">
                <svg
                  className="mx-auto h-8 w-8 text-[var(--text-disabled)] mb-2"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
          ...
          <div className="rounded-xl bg-blue-50 dark:bg-blue-900/20 px-3 py-2.5 text-[11px] text-blue-600 dark:text-blue-400 leading-relaxed">
            <span className="font-medium">提示：</span>允许代理到
            localhost、127.0.0.1、10.x、172.16-31.x、192.168.x 等内网地址。
          </div>
        </div>
