...
function getUpstreamBaseUrl(): string {
  const raw = process.env.LINKMIND_BASE_URL || 'http://localhost:8080'
    ...
    const req = client.request(options, res => {
      ...
      if (status >= 400) {
        ...
        res.on('end', () => {
          if (resolved) return
          resolved = true
          resolve({ status, statusText, headers: responseHeaders, body: Buffer.concat(chunks) })
        })
        res.on('error', err => {
          if (resolved) return
          resolved = true
      ...
      if (resolved) return
      resolved = true
      resolve({ status, statusText, headers: responseHeaders, bodyStream: res as unknown as Readable })
    })
...
async function proxy(request: Request, pathParts: string[]): Promise<Response> {
  ...
  const upstreamPath = `/${pathParts.map(p => encodeURIComponent(p)).join('/')}`
  ...
  const requestHeaders: Record<string, string> = {
    ...
    'User-Agent': 'doubao-linkmind-proxy',
  }
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          upstreamUrl,
          detail: detail.length > 4096 ? `${detail.slice(0, 4096)}...(truncated)` : detail,
        },
        { status: result.status, headers: { 'x-doubao-linkmind-proxy': '1' } }
      )
    }

    const responseHeaders = new Headers()
    responseHeaders.set('x-doubao-linkmind-proxy', '1')
    ...
    const message =
      typeof anyError?.message === 'string' ? anyError.message : 'Upstream fetch failed'
    return NextResponse.json(
      { success: false, error: message, upstream: upstreamBaseUrl },
      { status: 502 }
    )
  }
}
