...
const isNativeAudioModel = (modelId: string | null | undefined): boolean => {
    if (!modelId) return false;
    const lowerId = modelId.toLowerCase();
    return lowerId.includes('native-audio') || lowerId.includes('-live-');
};

const isGemini31FlashLiveModel = (modelId: string | null | undefined): boolean =>
    !!modelId && modelId.toLowerCase().includes('gemini-3.1-flash-live');

const isGemini31FlashImageModel = (modelId: string | null | undefined): boolean =>
    !!modelId && modelId.toLowerCase().includes('gemini-3.1-flash-image');

export const isGemmaModel = (modelId: string | null | undefined): boolean =>
    !!modelId && modelId.toLowerCase().includes('gemma');

const isTtsModel = (modelId: string | null | undefined): boolean => !!modelId && modelId.toLowerCase().includes('tts');

const isGemini3ImageModel = (modelId: string | null | undefined): boolean => (
    modelId === 'gemini-3-pro-image-preview' || modelId === 'gemini-3.1-flash-image-preview'
);

const isFlashImageModel = (modelId: string | null | undefined): boolean => !!modelId && modelId.toLowerCase().includes('gemini-2.5-flash-image');

const isRealImagenModel = (modelId: string | null | undefined): boolean => !!modelId && modelId.toLowerCase().includes('imagen');
...
export const getModelCapabilities = (modelId: string | null | undefined) => {
    const safeModelId = modelId || 'unknown';
    const isGemini3 = isGemini3Model(safeModelId);
    const gemini3ImageModel = isGemini3ImageModel(safeModelId);
    const flashImageModel = isFlashImageModel(safeModelId);
    const realImagenModel = isRealImagenModel(safeModelId);
    const ttsModel = isTtsModel(safeModelId);
    const nativeAudioModel = isNativeAudioModel(safeModelId);
    const imageModel = realImagenModel || flashImageModel || gemini3ImageModel;

    let supportedAspectRatios: string[] | undefined;
    if (realImagenModel) {
        supportedAspectRatios = ['1:1', '16:9', '9:16', '4:3', '3:4'];
    } else if (isGemini31FlashImageModel(safeModelId)) {
    ...
    if (isGemini31FlashImageModel(safeModelId)) {
        supportedImageSizes = ['512', '1K', '2K', '4K'];
    } else if (gemini3ImageModel) {
        supportedImageSizes = ['1K', '2K', '4K'];
    } else if (realImagenModel && !safeModelId.toLowerCase().includes('fast')) {
        supportedImageSizes = ['1K', '2K'];
    }

    return {
        isGemini3,
        isGemmaModel: isGemmaModel(safeModelId),
