...
import { DEEP_CRAWL_MCP_INSTRUCTION, getDeepCrawlPrompt } from "@/services/deep-crawl"
import { normalizeImageToDataUrl, normalizeImagesToDataUrls } from "@/utils/to-source"

const DEEP_CRAWL_IMAGE_LIMITS = {
  maxImages: 4,
...
export const useMessage = () => {
  ...
  const normalChatMode = async (
    ...
    const processedImages =
      images?.length && images.length > 0
        ? await normalizeImagesToDataUrls(images)
        : image.length > 0
          ? [image]
          : []
  ...
  const presetChatMode = async (
    ...
    const imageNormalizationOptions =
      messageType === "deep-crawl" ? DEEP_CRAWL_IMAGE_LIMITS : undefined
    const processedImages =
      images?.length && images.length > 0
        ? await normalizeImagesToDataUrls(images, imageNormalizationOptions)
        : image.length > 0
          ? [image]
          : []
    ...
    try {
      const prompt =
        messageType === "deep-crawl"
          ? await getDeepCrawlPrompt()
          : await getPrompt(messageType)
      const formattedPrompt =
        messageType === "deep-crawl"
          ? `${prompt}\n\n以下是全站深度爬取得到的正文内容，请基于这些内容完成分析：\n${message}${
              processedImages.length > 0
                ? `\n\n本次同时附带了 ${processedImages.length} 张页面图片，这些图片已经提供给你，请直接结合图片可见内容进行分析，不要回答“未分析图片内容”“无法看到图片”或仅根据图片数量做推断。`
                : ""
            }`
          : prompt.replace("{text}", message)
      let humanMessage = await humanMessageFormatter({
        content: [
          {
  ...
  const onSubmit = async ({
    ...
    if (messageType === "deep-crawl") {
      const deepCrawlPrompt = await getDeepCrawlPrompt()
      ...
      await sharedNormalChatMode(
        ...
        signal,
        {
          ...
          systemPromptOverride: [
            deepCrawlPrompt,
            DEEP_CRAWL_MCP_INSTRUCTION,
            "用户消息是全站深度爬取得到的网页正文材料；若附带图片，请直接结合图片内容一起分析，并在需要时继续调用可用的 MCP 工具补充网页信息。"
          ].join("\n\n"),
          imageNormalizationOptions: DEEP_CRAWL_IMAGE_LIMITS
        }
      )
      return
    }
