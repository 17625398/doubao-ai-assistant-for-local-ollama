...
import { addHistoryEntry, readHistory, searchHistory, clearHistory, getHistoryEntry } from './history.js';
import { commandQueue, type QueuedCommand } from './command-queue.js';
...
export function createProgram(BUILTIN_CLIS: string, USER_CLIS: string): Command {
  ...
  function operateAction(fn: (page: Awaited<ReturnType<typeof getOperatePage>>, ...args: any[]) => Promise<unknown>) {
    return async (...args: any[]) => {
      try {
        UI.startLoading('Connecting to browser...');
        const page = await getOperatePage();
        UI.stopLoadingSuccess('Browser connected');
        await fn(page, ...args);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        if (msg.includes('Extension not connected') || msg.includes('Daemon')) {
          UI.stopLoadingError('Browser not connected');
          UI.showTip('Run \'opencli doctor\' to diagnose');
        } else if (msg.includes('attach failed') || msg.includes('chrome-extension://')) {
          UI.stopLoadingError('Browser attach failed');
          UI.showTip('Another extension may be interfering. Try disabling 1Password.');
        } else {
          UI.stopLoadingError(`Error: ${msg}`);
        }
...
`;
  ...
  const record = operate.command('record').description('Record browser actions and convert to OpenCLI commands');

  record.command('start').description('Start recording browser actions')
    .action(operateAction(async (page) => {
      const { startRecording } = await import('./record-actions.js');
      await startRecording(page);
    }));

  record.command('pause').argument('<sessionId>', 'Session ID')
    .description('Pause recording')
    .action(operateAction(async (page, sessionId) => {
      const { pauseRecording } = await import('./record-actions.js');
      pauseRecording(sessionId);
    }));

  record.command('resume').argument('<sessionId>', 'Session ID')
    .description('Resume recording')
    .action(operateAction(async (page, sessionId) => {
      const { resumeRecording } = await import('./record-actions.js');
      resumeRecording(sessionId);
    }));

  record.command('stop').argument('<sessionId>', 'Session ID')
    .description('Stop recording and generate OpenCLI commands')
    .action(operateAction(async (page, sessionId) => {
      const { stopRecording } = await import('./record-actions.js');
      const commands = await stopRecording(page, sessionId);
      if (commands.length > 0) {
        console.log(chalk.bold('Generated OpenCLI commands:'));
        console.log();
  ...
  record.command('list').description('List active recording sessions')
    .action(operateAction(async (page) => {
      const { listRecordingSessions } = await import('./record-actions.js');
      const sessions = listRecordingSessions();
      if (sessions.length === 0) {
        console.log('No active recording sessions');
        return;
      }
      console.log(chalk.bold('Active recording sessions:'));
      console.log();
      sessions.forEach(session => {
        console.log(`  Session ID: ${session.id}`);
        console.log(`  Start Time: ${new Date(session.startTime).toLocaleString()}`);
        console.log(`  Status: ${session.isPaused ? 'Paused' : 'Recording'}`);
        console.log(`  Actions: ${session.actions.length}`);
        console.log(`  URL: ${session.pageUrl}`);
        console.log();
      });
    }));
  ...
  const historyCmd = program.command('history').description('Manage command history');
  ...
  historyCmd
    ...
    .action((opts) => {
      ...
      const count = parseInt(opts.number, 10);
      ...
      if (entries.length === 0) {
        UI.info('No command history available');
        return;
      }
      ...
      entries.forEach((entry, index) => {
        ...
        console.log(`  ${chalk.dim(`[${entries.length - index}]`)} ${status} ${chalk.dim(timestamp)}`);
  ...
  historyCmd
    ...
    .action((query) => {
      ...
      UI.showSeparator();
      UI.info(`Search Results for "${query}"`);
      UI.showSeparator();
      
      if (results.length === 0) {
        UI.info('No matching commands found');
        return;
      }
      ...
      results.forEach((entry, index) => {
        ...
        const status = entry.success ? chalk.green('✓') : chalk.red('✗');
        const fullCommand = `${entry.command} ${entry.args.join(' ')}`;
        console.log(`  ${chalk.dim(`[${index + 1}]`)} ${status} ${chalk.dim(timestamp)}`);
        console.log(`    ${fullCommand}`);
        console.log();
      });
    });
  ...
  historyCmd
    ...
    .action(() => {
      clearHistory();
      UI.success('Command history cleared');
    });

  historyCmd
    .command('run')
    .description('Re-run a command from history')
    .argument('<index>', 'History entry index')
    .action((index) => {
      const entry = getHistoryEntry(parseInt(index, 10) - 1);
      if (!entry) {
        UI.error('History entry not found');
        process.exitCode = EXIT_CODES.USAGE_ERROR;
        return;
      }
      
      const fullCommand = `${entry.command} ${entry.args.join(' ')}`;
      UI.success(`Re-running: ${fullCommand}`);
      
      // Execute the command
      try {
        UI.startLoading('Executing command...');
        execSync(`npx tsx src/main.ts ${fullCommand}`, { stdio: 'inherit' });
        UI.stopLoadingSuccess('Command executed successfully');
      } catch (error) {
        UI.stopLoadingError('Command failed to execute');
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
    });
  ...
  pluginCmd
    ...
    .action(async (source: string) => {
      ...
      try {
        UI.startLoading('Installing plugin...');
        const result = installPlugin(source);
        await discoverPlugins();
        if (Array.isArray(result)) {
          if (result.length === 0) {
            UI.stopLoadingWarn('No plugins were installed (all skipped or incompatible).');
          } else {
            UI.stopLoadingSuccess(`Installed ${result.length} plugin(s) from monorepo: ${result.join(', ')}`);
          }
        } else {
          UI.stopLoadingSuccess(`Plugin "${result}" installed successfully. Commands are ready to use.`);
        }
      } catch (err) {
        UI.stopLoadingError(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
    });
  ...
  pluginCmd
    ...
    .action(async (name: string) => {
      ...
      try {
        UI.startLoading('Uninstalling plugin...');
        uninstallPlugin(name);
        UI.stopLoadingSuccess(`Plugin "${name}" uninstalled.`);
      } catch (err) {
        UI.stopLoadingError(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  pluginCmd
    ...
    .action(async (name: string | undefined, opts: { all?: boolean }) => {
      if (!name && !opts.all) {
        UI.error('Please specify a plugin name or use the --all flag.');
        process.exitCode = EXIT_CODES.USAGE_ERROR;
        return;
      }
      if (name && opts.all) {
        UI.error('Cannot specify both a plugin name and --all.');
        process.exitCode = EXIT_CODES.USAGE_ERROR;
        return;
      }
      ...
      const { discoverPlugins } = await import('./discovery.js');
      if (opts.all) {
        UI.startLoading('Updating all plugins...');
        const results = updateAllPlugins();
        if (results.length > 0) {
          await discoverPlugins();
        }
        UI.stopLoadingSuccess('Plugin update completed');
        ...
        console.log();
        if (hasErrors) {
          UI.error('Completed with some errors.');
          process.exitCode = EXIT_CODES.GENERIC_ERROR;
        } else {
          UI.success('All plugins updated successfully.');
        }
        return;
      }

      try {
        UI.startLoading(`Updating plugin "${name}"...`);
        updatePlugin(name!);
        await discoverPlugins();
        UI.stopLoadingSuccess(`Plugin "${name}" updated successfully.`);
      } catch (err) {
        UI.stopLoadingError(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  pluginCmd
    ...
    .action(async (opts) => {
      ...
      if (plugins.length === 0) {
        UI.info('No plugins installed.');
        UI.showTip('Install one with: opencli plugin install github:user/repo');
        return;
      }
      if (opts.format === 'json') {
      ...
      console.log();
      UI.info(`${plugins.length} plugin(s) installed`);
      UI.showSeparator();
    });
  ...
  pluginCmd
    ...
    .action(async (name: string, opts: { dir?: string; description?: string }) => {
      ...
      try {
        UI.startLoading('Creating plugin scaffold...');
        const result = createPluginScaffold(name, {
          dir: opts.dir,
          description: opts.description,
        });
        UI.stopLoadingSuccess(`Plugin scaffold created at ${result.dir}`);
        
        UI.showSeparator();
        UI.info('Files created:');
  ...
  const queueCmd = program.command('queue').description('Manage command queue');
  
  queueCmd
    .command('add')
    .description('Add a command to the queue')
    .argument('<command>', 'Command to add (e.g. google/search)')
    .option('--args <args>', 'Command arguments as JSON string')
    .option('--debug', 'Enable debug mode')
    .action(async (command: string, opts: { args?: string; debug?: boolean }) => {
      try {
        UI.startLoading('Adding command to queue...');
        const parts = command.split('/');
        if (parts.length < 2) {
          UI.stopLoadingError('Command must be in site/command format (e.g. google/search)');
          process.exitCode = EXIT_CODES.USAGE_ERROR;
          return;
        }
        
        const [site, cmdName] = parts;
        const registry = getRegistry();
        const cmd = registry.get(`${site}/${cmdName}`);
        
        if (!cmd) {
          UI.stopLoadingError(`Command not found: ${command}`);
          process.exitCode = EXIT_CODES.USAGE_ERROR;
          return;
        }
        ...
        const id = commandQueue.add(cmd, args, debug);
        UI.stopLoadingSuccess(`Added command ${command} to queue with id ${id}`);
      } catch (error) {
        UI.stopLoadingError(`Error: ${error instanceof Error ? error.message : String(error)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
    });
  ...
  queueCmd
    .command('add-batch')
    .description('Add multiple commands to the queue')
    .argument('<commands>', 'Commands as JSON array')
    .action(async (commands: string, opts) => {
      try {
        UI.startLoading('Adding multiple commands to queue...');
        const commandsArray = JSON.parse(commands);
        if (!Array.isArray(commandsArray)) {
          UI.stopLoadingError('Commands must be a JSON array');
          process.exitCode = EXIT_CODES.USAGE_ERROR;
          return;
        }
        ...
        for (const cmdData of commandsArray) {
          if (!cmdData.command) {
            UI.stopLoadingError('Each command must have a "command" property');
            process.exitCode = EXIT_CODES.USAGE_ERROR;
            return;
          }
          
          const parts = cmdData.command.split('/');
          if (parts.length < 2) {
            UI.stopLoadingError(`Command must be in site/command format: ${cmdData.command}`);
            process.exitCode = EXIT_CODES.USAGE_ERROR;
            return;
          }
          ...
          if (!cmd) {
            UI.stopLoadingError(`Command not found: ${cmdData.command}`);
            process.exitCode = EXIT_CODES.USAGE_ERROR;
            return;
          }
          ...
          validCommands.push({
            command: cmd,
            args: cmdData.args ?? {},
            debug: cmdData.debug ?? false
          });
        }
        
        const ids = commandQueue.addBatch(validCommands);
        UI.stopLoadingSuccess(`Added ${ids.length} commands to queue`);
        console.log(`Command IDs: ${ids.join(', ')}`);
      } catch (error) {
        UI.stopLoadingError(`Error: ${error instanceof Error ? error.message : String(error)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  queueCmd
    ...
    .action(() => {
      commandQueue.pause();
      UI.success('Command queue paused');
    });
  
  queueCmd
    .command('resume')
    .description('Resume the command queue')
    .action(() => {
      commandQueue.resume();
      UI.success('Command queue resumed');
    });
  
  queueCmd
    .command('cancel')
    .description('Cancel the entire command queue')
    .action(() => {
      commandQueue.cancel();
      UI.success('Command queue cancelled');
    });
  ...
  queueCmd
    ...
    .action(() => {
      ...
      if (status.currentCommand) {
        console.log(`  Current Command: ${fullName(status.currentCommand.command)} (${status.currentCommand.id})`);
        console.log(`  Status: ${status.currentCommand.status}`);
      } else {
        console.log(`  Current Command: None`);
      }
      
      if (status.queue.length > 0) {
        console.log('  Queue:');
        status.queue.forEach((cmd: QueuedCommand, index: number) => {
          console.log(`    ${index + 1}. ${fullName(cmd.command)} (${cmd.id}) - ${cmd.status}`);
        });
      } else {
  ...
  queueCmd
    ...
    .action(async () => {
      UI.startLoading('Waiting for queue to complete...');
      try {
        await commandQueue.waitForCompletion();
        UI.stopLoadingSuccess('Queue completed');
      } catch (error) {
        UI.stopLoadingError(`Error: ${error instanceof Error ? error.message : String(error)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  program.on('command:*', (operands: string[]) => {
    ...
    UI.error(`Unknown command '${binary}'`);
    if (isBinaryInstalled(binary)) {
      UI.showTip(`'${binary}' exists on your PATH. Use 'opencli register ${binary}' to add it as an external CLI.`);
    }
    program.outputHelp();
    process.exitCode = EXIT_CODES.USAGE_ERROR;
  });
...
export function runCli(BUILTIN_CLIS: string, USER_CLIS: string): void {
  ...
  // Capture command before parsing
