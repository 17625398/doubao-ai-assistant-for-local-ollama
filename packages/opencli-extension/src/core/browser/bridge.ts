...
export class BrowserBridge implements IBrowserFactory {
  ...
  private async _ensureDaemon(timeoutSeconds?: number): Promise<void> {
    ...
    let status = await fetchDaemonStatus();
    ...
    try {
      ...
      // Set up error handling for the daemon process
      this._daemonProc.on('error', (err) => {
        if (process.env.OPENCLI_VERBOSE) {
          process.stderr.write(`⚠️  Daemon process error: ${err.message}\n`);
        }
      });
      
      this._daemonProc.on('exit', (code, signal) => {
        if (process.env.OPENCLI_VERBOSE) {
          process.stderr.write(`⚠️  Daemon process exited with code ${code}, signal ${signal}\n`);
        }
      ...
      while (Date.now() < deadline) {
        ...
        // Periodically check if daemon is still running
        if (attempts % 5 === 0) { // Check every second
          status = await fetchDaemonStatus();
          if (status === null) {
            // Daemon crashed, try to restart
            if (process.env.OPENCLI_VERBOSE) {
              process.stderr.write('⚠️  Daemon crashed, restarting...\n');
            }
      ...
      // Clean up if daemon process was spawned
      if (this._daemonProc) {
        try {
          this._daemonProc.kill();
        } catch (e) {
