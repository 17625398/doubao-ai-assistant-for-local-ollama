...
export class TimeoutError extends CliError {
  constructor(label: string, seconds: number, hint?: string) {
    super(
      ...
      hint ?? 'Try again, or increase timeout with OPENCLI_BROWSER_COMMAND_TIMEOUT env var. Check your internet connection and try again.',
      EXIT_CODES.TEMPFAIL,
    );
  }
}
...
export class ApiError extends CliError {
  constructor(message: string, hint?: string) {
    super(
      'API_ERROR',
      message,
      hint ?? 'The service API may be temporarily unavailable. Try again later.',
      EXIT_CODES.SERVICE_UNAVAIL,
    );
  }
}
...
export class RateLimitedError extends CliError {
  constructor(message: string, hint?: string) {
    super(
      'RATE_LIMITED',
      message,
      hint ?? 'You may be rate limited. Try again later or reduce the frequency of your requests.',
      EXIT_CODES.TEMPFAIL,
    );
  }
}
...
export class PageChangedError extends CliError {
  constructor(message: string, hint?: string) {
    super(
      'PAGE_CHANGED',
      message,
      hint ?? 'The page structure has changed. Please update the adapter or report this issue.',
