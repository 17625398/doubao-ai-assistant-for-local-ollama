...
import { AdapterLoadError, ArgumentError, BrowserConnectError, CommandExecutionError, TimeoutError, getErrorMessage } from './errors.js';
...
// Global module cache to avoid重复加载 across command executions
export const _loadedModules = new Set<string>();
...
export async function executeCommand(
  ...
  let result: unknown;
  let retries = 0;
  const maxRetries = cmd.maxRetries ?? 0;
  ...
  if (isTTY && !debug) {
    process.stdout.write(`🔄 Executing ${commandName}... `);
  }
  ...
  while (true) {
    try {
      if (shouldUseBrowserSession(cmd)) {
        ...
        result = await browserSession(BrowserFactory, async (page) => {
          ...
          if (preNavUrl) {
            ...
            try {
              if (isTTY && !debug) {
                process.stdout.write(`\r🔄 Executing ${commandName}... Navigating to ${preNavUrl} `);
              }
          ...
          if (isTTY && !debug) {
            process.stdout.write(`\r🔄 Executing ${commandName}... Running command `);
          }
      ...
      if (isTTY && !debug) {
        process.stdout.write(`\r✅ Executing ${commandName}... Done\n`);
      }
      
      break;
    } catch (err) {
      // Check if we should retry
      if (retries < maxRetries && (err instanceof BrowserConnectError || err instanceof TimeoutError)) {
        retries++;
        if (debug) log.debug(`[retry] Command ${fullName(cmd)} failed, retrying (${retries}/${maxRetries})...`);
        if (isTTY && !debug) {
          process.stdout.write(`\r🔄 Executing ${commandName}... Retrying (${retries}/${maxRetries}) `);
        }
      ...
      if (isTTY && !debug) {
        process.stdout.write(`\r❌ Executing ${commandName}... Failed\n`);
      }
      
      // Not retrying or retry failed
      hookCtx.error = err;
